...
def entrada_general(formulario, tablas, actualicion):
    labels_texts = [
        "Ingrese el color:",
        "Ingrese el tiempo inicio (hora):",
        "Ingrese el tiempo fin (hora):",
        "Ingrese la placa:",
        "Ingrese el género:"
    ]
    entries = {}
    ...
    for texto in labels_texts:
        lbl = tkinter.Label(formulario, text=texto)
        lbl.pack(pady=5)
        ent = tkinter.Entry(formulario, bd=1, relief="solid")
        ent.pack(pady=5)
        entries[texto] = ent
    ...
    def boton_filtrar():
        ...
        if tiempo_inicio and tiempo_fin:
            if not tiempo_inicio.isdigit() or not tiempo_fin.isdigit():
                messagebox.showerror("‼Error", "Los valores de tiempo deben ser numéricos")
                return
    ...
    boton = tkinter.Button(formulario, text="Filtrar", command=boton_filtrar)
    boton.pack(pady=10)
